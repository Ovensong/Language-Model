...
for i in range(len(words) - 2):
    pairs_of_words.append([words[i], words[i + 1], words[i + 2]])

# Randomly pick next word
def next_word(last_word, last_last_word):
    possible = []
...
try:
    ...
    for i in range(amount_of_words - len(output)):
        ...
        output.append(next_word(output[len(output) - 1], output[len(output) - 2]))
